...
* [ ] Connect Argon to MQTT broker
* [ ] Connect MongoDB program to broker
...
The next step in this cycle is connecting the particle Argon to a broker. For this, I decided to use MQTTHQ as a my client as it didn't require any additional authentication which could cause issues when connected. I started by adding the following code to my origonal data collection program.
...
void mqtt_publish(char *pubdata) {
  ...
  client.publish("lucytopic","test point 1");
...
<strong>  client.connect(clientId);
    ...
    client.publish("lucytopic","test point 2");
  }
}
...
void loop(){
  ...
  if (!client.isConnected()) {
    ...
    client.publish("lucytopic","test point 3");
...
In this development cycle, I am going to focus on combining section [2.1 ](2.2.2-cycle-2-storing-data.md#2.1)and [2.2](2.2.2-cycle-2-storing-data.md#2.2) to design a system that connects to the weather station and uploads this data to the database.
