...
As expected, these changes added the correct date and time to the file. This test has now passed.
...
| ---- | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 1    | Adapt the C++ program in cycle 1 to connect with an MQTT client | The program to connect to MQTTHQ and be able to publish weather readings                                                                                                                           | I subscribed to the MQTT client in the terminal and recieved weather data as expected at regular intervals                                                                                                                                            | Pass      |
| 2    | Design a program that can upload data to mongoDB in python      | The python program should connect to mongo database and carry out some basic operations, including inserting, updating and deleting data                                                           | The data was recieved and stored on mongoDB                                                                                                                                                                                                           | Pass      |
| 3    | Connect the python program the the MQTT client                  | For this test, I expect the python program to recieve weather data and then print it in the terminal. It should then make any necessary alterations (e.g. adding a date and timestamp to the data) | The program ran and recieved data values from the weather station. The date and time were added to the dictionary using the .update() method                                                                                                          | Pass      |
| 4    | Upload weather data to database                                 | The next test is to ensure the weather data is stored on mongoDB in the correct format                                                                                                             | At first I had an issue with the MQTT data recieved being the incorrect type (plain text instead of a dictonary). However, after casting the variable type the code ran correctly and weather readings from the sensors were recieved on the database | Pass      |
