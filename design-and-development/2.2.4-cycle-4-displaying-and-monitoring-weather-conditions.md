...
* [x] Display other basic information on the user interface
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.1 | Automatically updating the user interface                                                                                                                | In this part, I will focus on adapting my react app to update its components at regular intervals (e.g. 30 seconds).                                                                                       | The page history should update with new readings within 30 seconds of them being received by the database.                                                                                                            |
...
| 4.4 | Display any other basic information, such as the time since the last reading and the time the weather station was first activated on the user interface. | This section will involve altering the "About Your Weather Station" page to contain information such as whether connection to the weather station has been successful and when the last reading was taken. | Use data to display a connection status, when data was last received and when the weather station was first activated.                                                                                                |

## Data structures

| Structure Name     | Structure Type   | Description                                                                                |
| ------------------ | ---------------- | ------------------------------------------------------------------------------------------ |
| message            | Array Variable   | Contains objects containing all the weather data collected by the weather station          |
| interval           | Number           | Stores the interval in milliseconds between updates on the user interface                  |
| connection\_status | String Variable  | Stores either "ACTIVE" or "OFFLINE" based on whether data has been received within 2 hours |
| dateTimestamp      | Integer Variable | Stores the timestamp of the most recent reading                                            |
| nowTimestamp       | Integer Variable | Stores the current timestamp                                                               |
| quality            | String Variable  | Contains the current air quality                                                           |
| qual               | Integer Variable | Stores a number equivalent to the air quality to determine the position of the speedometer |
| data               | Object           | Contains the data for graphs and charts                                                    |
| options            | Object           | Contains the settings for the graphs and charts                                            |
| timestamp          | Integer Variable | Stores the timestamp of the most recent reading                                            |
| dust               | Integer Variable | Stores the dust concentration collected in the most recent reading                         |
...
As expected, this allowed new data to be displayed on the react app without requiring the back-end to restart. However, after refreshing the app again, I received the following error.
...
The next step is to ensure that the front-end app does not need to be manually refreshed to receive the most recent weather conditions. Instead, it should automatically update its components. To do this, I used the setInterval() method to get the weather data every 30 seconds.&#x20;
...
Next, I am going to display the current air quality on the user interface. I started by displaying the basic reading (e.g. fresh air, low pollution, danger).
...
As expected, this produced a progress bar which automatically refreshes itself when it receives new data.
...
While the code successfully produced a graph containing the 24 most recent readings of temperature, I realised that since in the database data is sorted from most recent to oldest, all the readings were in the reverse order.
...
As well as containing basic information about how the weather station works, the about page will display 3 pieces of data; a connection status, when data was last received, and when the weather station was first activated.&#x20;
...
```javascript
<div className="aboutInfoContent" ><div className="aboutLog"><p>Weather data last received - <b>{message?.weatherData?.slice(0,1).map((item, idx)=>{
    return (
        <div key={idx} > 
        {new Date(item.timestamp*1000).toLocaleDateString()}&nbsp;at&nbsp;{new Date(item.timestamp*1000).toLocaleTimeString()}
        </div>
...
```javascript
<div className="aboutInfoContent" ><div className="aboutLog"><p>Weather data last received - <b>{message?.weatherData?reverse().slice(0,1).map((item, idx)=>
    return (
        <div key={idx} > 
        {new Date(item.timestamp*1000).toLocaleDateString()}&nbsp;at&nbsp;{new Date(item.timestamp*1000).toLocaleTimeString()}
        </div>
...
The last page I need to complete is the help page. This will contain all the instructions needed by the user to help them to understand how to use this weather monitoring system. This can be seen below.
...
<figure><img src="../.gitbook/assets/Screenshot 2023-08-20 at 13.25.46.png" alt="" width="563"><figcaption></figcaption></figure>

### Outcome

Overall, in this cycle, I used the data collected by my weather station and displayed it visually on my user interface, an app. Firstly I adapted the back-end server and client program to collect data at regular intervals and refresh automatically. I then designed a program that displays temperature, dust concentration, humidity, air pressure, sound, light and altitude. Using line graphs, area graphs and bar charts, I then displayed trends in the weather data over time. The results of this cycle can be seen below.
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 1    | Adapt the app to automatically update its contents at regular intervals. (see [4.1](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.1))                         | When new data is recorded on the database, the app should be updated within 30 seconds.                                                        | The app successfully updated itself upon receiving new weather data.                                                                   | Pass      |
| 2    | Display basic readings (see [4.2](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.2), [4.3](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.3)) | Readings should be displayed visually in a format that is easily accessible to the user.                                                       | Displayed temperature, dust concentration, air pressure and altitude on the user interface.                                            | Pass      |
| 3    | Use a progress bar to show the relative proportion of data (e.g. percentage humidity) (see [4.3](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.3))            | Readings for sound, light and humidity should be shown on a user interface and automatically update itself.                                    | The app correctly displayed readings for sound, light and humidity.                                                                    | Pass      |
| 4    | Air quality index (see [4.2](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.2))                                                                                | The needle should change direction and point at the appropriate air quality.                                                                   | I was successfully able to display air quality readings on a speedometer to represent changes in air quality.                          | Pass      |
| 5    | Tracking trends in weather data (see [4.3](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.3))                                                                  | In this section I will use line graphs, area graphs and bar charts to visualise trends in data.                                                | Successfully programmed charts to track trends in temperature, a bar chart comparing light and sound, humidity and dust concentration. | Pass      |
| 6    | About your weather station page (see [4.4](2.2.4-cycle-4-displaying-and-monitoring-weather-conditions.md#4.4))                                                                  | The app should display connection status, the time the last piece of weather data was received and when the weather station was first updated. | All the readings were correctly displayed on the app.                                                                                  | Pass      |
