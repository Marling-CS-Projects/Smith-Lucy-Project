...
While I currently only have a limited number of readings stored in the database, when the weather station is in use for long periods of time, the database will store much larger numbers of readings, which is too many to display in the weather history. Therefore, I will set the maximum number of log enteries to 50.

To test this, I will start by limiting the readings to four.

```javascript
message?.weatherData?.slice(0,4).map((item)
```
...
I then changed the .slice() method range to (0,50), limiting the number of readings to 50.
